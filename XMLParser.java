...
public class XMLParser {
    public static void main(String[] args) {
        try {
            File inputFile = new File("patient_data (1).xml"); // Replace with your XML file path
            DocumentBuilderFactory dbFactory = DocumentBuilderFactory.newInstance();
            DocumentBuilder dBuilder = dbFactory.newDocumentBuilder();
            Document doc = dBuilder.parse(inputFile);
            doc.getDocumentElement().normalize();

            NodeList nodeList = doc.getElementsByTagName("patient_data");
            ...
            for (int i = 0; i < nodeList.getLength(); i++) {
                ...
                if (node.getNodeType() == Node.ELEMENT_NODE) {
                    ...
                    String price = element.getElementsByTagName("age").item(0).getTextContent();
                    String description = element.getElementsByTagName("condition").item(0).getTextContent();
